...
// ************** this file complete and generate a calculated table , and calculated columns from excel

//string fileName = @"C:\Desktop\ModelAutoBuild"; // Enter Model Auto Build Excel file
string myWorkbook = @"C:\modul\5.xlsx";
var excelApp = new Excel.Application();
excelApp.Visible = false;
excelApp.DisplayAlerts = false;
Excel.Workbook wb = excelApp.Workbooks.Open(myWorkbook);

string[] tabs = {"DataSources", "Tables", "MeasuresColumns", "Relationships", "Model", "Roles", "RLS", "Hierarchies", "CalculatedTables"};
int tabCount = tabs.Count();
string[] tableTypes = {"CALC","FACT","DIM","BRIDGE","SEC","META"};
...
for (int i=0; i<tabCount; i++)
{
    ...
    for (int r=2; r<=rowCount; r++)
    {
        ...
        {           
            ...
            string expr = (string)(ws.Cells[r,8] as Excel.Range).Text.ToString();           
        ...
        {
            string tableName = (string)(ws.Cells[r,1] as Excel.Range).Text.ToString();
            string expr = (string)(ws.Cells[r,2] as Excel.Range).Text.ToString(); 
            var table = Model.AddCalculatedTable(tableName, "{0}");
            var column = table.AddCalculatedTableColumn(tableName, expr, "", DataType.Int64);
            table.Expression = expr;
