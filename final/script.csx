...
string myWorkbook = @"C:\template.xlsx";
var excelApp = new Excel.Application();
excelApp.Visible = false;
excelApp.DisplayAlerts = false;
Excel.Workbook wb = excelApp.Workbooks.Open(myWorkbook);

string[] tabs = {"DataSources", "Tables", "MeasuresColumns", "Relationships", "Model", "Roles", "RLS", "Calc"};
...
for (int i=0; i<tabCount; i++)
{
    ...
    if (i==8 )
    {
        var table = Model.AddCalculatedTable("8", "{0}");
        var column = table.AddCalculatedTableColumn("Value", "[Value]", "", DataType.Int64);
    }
